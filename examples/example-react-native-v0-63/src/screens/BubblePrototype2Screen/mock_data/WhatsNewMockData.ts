...
import { WhatsNewConsolidatedData, WhatsNewEntry } from '../WhatsNewService';
...
type PartialWhatsNewEntry = Pick<WhatsNewEntry, 'articleTitle' | 'promoID'>;
...
const MOCK_WHATS_NEW_ENTRY_PARTIAL: Record<MockKey, PartialWhatsNewEntry> = {
  ...
  mock06: {
    promoID:
      'https://www.cimb.com/en/newsroom/2025/cimb-commits-rm10-billion-to-accelerate-growth-in-johor-singapore-special-economic-zone.html',
    articleTitle: 'Grow!',
  },
};
...
export function getMockImageLocalURIFromMockImageURL(
  mockUrl: string,
): string | undefined {
...
export function replaceMockImageURLWithLocalMockImageURI(
  payload: WhatsNewConsolidatedData,
) {
  ...
  for (let index = 0; index < whatsNewEntries.length; index++) {
    ...
    const localPathURIForImagePreviewURL =
      getMockImageLocalURIFromMockImageURL(imagePreviewMockURL);
