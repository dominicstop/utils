...
import { WhatsNewPayload, WhatsNewServiceShared } from "./WhatsNewService";
import { LoadingState, LoadingStateWithData, useAsyncLazyRef, useLazyRef } from "./Temp";
import { MOCK_DEBUG_CONFIG } from "./mock_data/WhatsNewMockData";

export type WhatsNewDataState = LoadingStateWithData<WhatsNewPayload, string>
...
export function useWhatsNewData(args?: {
  ...
  const [_, resetWhatNewData] = useAsyncLazyRef(
    async () => {
      const data = await WhatsNewServiceShared.fetchWhatsNew({
