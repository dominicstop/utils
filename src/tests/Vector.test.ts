...
describe("Vector", () => {
  ...
  test("isZero === isUnit", () => {
    const zero = new Vector({ dx: 0, dy: 0 });
    const unit = new Vector({ dx: 1, dy: 0 });
  ...
  test("isEqualToOtherVector", () => {
    const a = new Vector({ dx: 1, dy: 1 });
    const b = new Vector({ dx: 1.00001, dy: 1.00001 });
  ...
  test("projectOntoOtherVector", () => {
    const a = new Vector({ dx: 3, dy: 4 });
    const b = new Vector({ dx: 1, dy: 0 });
  ...
  test("angleBetweenOtherVector", () => {
    const a = new Vector({ dx: 1, dy: 0 });
    const b = new Vector({ dx: 0, dy: 1 });
  ...
  test("static alias equality", () => {
    expect(Vector.zero.asValue).toEqual({ dx: 0, dy: 0 });
    expect(Vector.unitX.asValue).toEqual({ dx: 1, dy: 0 });
    expect(Vector.unitY.asValue).toEqual({ dx: 0, dy: 1 });
  });

  test("initFromAngle", () => {
    const angle = Angle.initFromDegrees(0);
    const v = Vector.initFromAngle(angle, 5);
  ...
  test("initFromPoints", () => {
    const v = Vector.initFromPoints({ x: 1, y: 2 }, { x: 4, y: 6 });
    expect(v.asValue).toEqual({ dx: 3, dy: 4 });
  });
  ...
  test("computeAverage", () => {
    ...
    const avg = Vector.computeAverage([a, b]);
  ...
  test("distanceBetweenTwoVectors", () => {
    ...
    expect(Vector.distanceBetweenTwoVectors(a, b)).toBeCloseTo(5);
  });
});
